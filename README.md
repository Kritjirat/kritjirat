[README (1).md](https://github.com/user-attachments/files/30451957/README.1.md)
<div align="center">

# Hi, I'm Kritjirat Thunkitjai 👋

### Mobile Developer (Flutter) • Full-Stack Developer • IT Student

*"Building apps that solve real problems — one screen and one API at a time."*

</div>

---

### 🚀 About Me

- 🎓 4th-year Information Technology student
- 📱 Passionate about **mobile development** (Flutter/Dart) and **full-stack development** (Spring Boot, PostgreSQL)
- 🏝️ Currently building **Wellness Tourism Route Planner** — a full-stack mobile app (Flutter + Spring Boot API) that helps travelers plan wellness-focused trips
- 🏆 3-time Academic Excellence Award recipient
- 🎯 2025/2026 goal: land an internship in **Mobile Development (Flutter)** or **Full-Stack Development**
- 📫 Reach me: [kritjirattkj@gmail.com](mailto:kritjirattkj@gmail.com)

---

### 🛠️ Tech Stack & Tools

| Domain       | Primary | Comfortable | Currently Exploring |
|--------------|---------|-------------|----------------------|
| **Mobile**   | | | ![Flutter](https://img.shields.io/badge/Flutter-02569B?style=flat&logo=flutter&logoColor=white) ![Dart](https://img.shields.io/badge/Dart-0175C2?style=flat&logo=dart&logoColor=white)   |
| **Backend**  | ![Spring Boot](https://img.shields.io/badge/Spring%20Boot-6DB33F?style=flat&logo=springboot&logoColor=white) ![Java](https://img.shields.io/badge/Java-ED8B00?style=flat&logo=openjdk&logoColor=white) | | |
| **Database** | ![SQL](https://img.shields.io/badge/SQL-4479A1?style=flat&logo=databricks&logoColor=white) | ![Oracle](https://img.shields.io/badge/Oracle-F80000?style=flat&logo=oracle&logoColor=white) | ![PostgreSQL](https://img.shields.io/badge/PostgreSQL-4169E1?style=flat&logo=postgresql&logoColor=white) ![SQLite](https://img.shields.io/badge/SQLite-003B57?style=flat&logo=sqlite&logoColor=white) |
| **Tools**    | ![Git](https://img.shields.io/badge/Git-F05032?style=flat&logo=git&logoColor=white) ![GitHub](https://img.shields.io/badge/GitHub-181717?style=flat&logo=github&logoColor=white) | | |

---

### 📌 Featured Project

**Wellness Tourism Route Planner** — A full-stack mobile application that helps users discover and plan wellness-oriented travel routes.
- 📱 **Frontend:** Flutter, custom UI components, responsive layouts
- ⚙️ **Backend:** Spring Boot REST API, JPA entities, DTO-based data transfer
- 🗄️ **Database:** PostgreSQL with relational route/district mapping design

<!-- Replace with your actual repo link -->
🔗 [View Repository](https://github.com/yourusername/wellness-tourism-planner)

---

<div align="center">

📫 **Let's connect:** [kritjirattkj@gmail.com](mailto:kritjirattkj@gmail.com)

</div>
